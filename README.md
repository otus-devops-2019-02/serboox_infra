...
1) Создал Vagrantfile, в место :virtualbox провайдера решил использовать libvirt KVM т.к. vagrant провайдер есть только для версии 5.2, а я давно на 6 версии)
2) Поднял две виртуалки используя box для libvirt "generic/ubuntu1604"
3) Добавил providion в appserver и dbserver, добавил raw модуль для установки python.
4) Перенес таски для packer в соответствующие ansible роли.
5*) Сделал открытие приложения на 80 порту
6) Создал окружение используя virtualenvwrapper и засетапил requirements.txt
7) Создал сценарий molecule, написал тест проверяющий доступность порта 27017, внес правки в шаблоны и плейбуки для packer, самостоятельно решил проблему падения билда.
8) В коре репозитория создал Makefile который с помошью sed: переносит название образов созданных packer в terraform, переносит ip адреса инстансов созданных terraform в ansible inventory and vars. Теперь можно получить готовое окружение просто выполнив команду make run.
